...
export default function DialerScreen({ phone, leads, onSelectLead, onOpenTimeline }: Props) {
  ...
  try {
    ...
    if (found) {
      ...
      // ✅ Load correct note — task_note if follow up, status_note if status
      if (found.taskName && found.taskName.startsWith("Follow Up:")) {
        setNote(found.task_note || "");
      } else {
        setNote(found.status_note || "");
      }

      if (found.follow_up_date) {
        setFollowUpDateDisplay(formatDisplayDate(found.follow_up_date));
      } else {
  ...
  try {
    ...
    if (pureStatuses.includes(newStatus)) {
      // ✅ status_note updated — task_note untouched
      await updateLeadStatusDB(phone, newStatus, undefined, note || undefined);
      setStatus(newStatus);
      await loadLead();

    } else if (newStatus.startsWith("Follow Up:")) {
      // ✅ task_note updated — status_note untouched
      await updateLeadStatusDB(phone, status, newStatus, note || undefined);
      await loadLead();
...
const styles = StyleSheet.create({
  ...
  noteBox: { backgroundColor: "#fff", marginTop: 20, borderRadius: 12, padding: 12, height: 160, textAlignVertical: "top" },
